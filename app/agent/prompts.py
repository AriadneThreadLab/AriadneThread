...
3. query_osm — live OpenStreetMap features via the Overpass API. Use it only for
   current geographic features. It is the only source of live map data. Successful
   results include a dataset_ref such as osm_result_1. One spatial target per call.
...
- Reply with ONLY one JSON object for this turn — either
  {"tool_calls":[{"name":"...","arguments":{...}}]} or {"final_answer":"..."}.
